...
# PART 1: backend.tf
# ─────────────────────────────────────────────────────────────────────────────
log_step "Setting up backend.tf (remote state configuration)"
echo ""
echo "  Terraform stores its state file (memory of what it created) in HCS OBS."
echo "  You need an OBS bucket that already exists for this."
echo "  Create it manually in the HCS console before continuing."
echo ""
...
if [[ "$SKIP_BACKEND" == false ]]; then
  ...
  echo "  Default: ${ENVIRONMENT}/terraform.tfstate"
  echo ""
...
###############################################################################
# backend.tf — Remote state configuration for ${ENVIRONMENT}
...
EOF
  ...
  log_success "backend.tf created: ${BACKEND_FILE}"
fi
